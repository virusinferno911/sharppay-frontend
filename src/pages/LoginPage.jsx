        ...
        <motion.form onSubmit={submit} initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} className="space-y-4">
          <div>
            ...
            <input type="email" value={form.email} onChange={set('email')}
              placeholder="you@example.com" required autoComplete="email" className="input-dark" />
          </div>
          <div>
            ...
            <div className="relative">
              <input type={showPw ? 'text' : 'password'} value={form.password}
                ...
                required className="input-dark pr-12" autoComplete="current-password" />
          ...
          <p className="text-center text-sm text-white/40 pt-1 pb-6">
            New to SharpPay?{' '}
            <Link to="/register" className="text-rose-400 font-semibold">Create account</Link>
          </p>
        </motion.form>
      </div>
    </div>
