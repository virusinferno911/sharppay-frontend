...
export default function CardsPage() {
  ...
  const handleFreezeToggle = async () => {
    ...
    // UI FIX: Instantly hide details if the user freezes the card
    if (isFreezing) {
    ...
    try {
      if (isFreezing) {
        await api.put('/cards/freeze');
      } else {
        await api.put('/cards/unfreeze');
      }
    } catch (err) {
  ...
  const handleConfirmPinComplete = (confirmPin) => {
        ...
        {loading ? (
          ...
          <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="space-y-6">
            ...
            <div className="grid grid-cols-2 gap-4 mt-6">
              {/* UI FIX: Button greys out and becomes unclickable if card is frozen */}
              <button 
                onClick={() => setShowDetails(!showDetails)} 
                disabled={card.status === 'FROZEN'}
                className={`bg-white rounded-2xl py-4 flex flex-col items-center justify-center gap-2 border border-purple-50 shadow-sm transition-colors ${card.status === 'FROZEN' ? 'opacity-50 cursor-not-allowed' : 'hover:bg-purple-50 active:scale-95'}`}
              >
                <div className={`w-10 h-10 rounded-full flex items-center justify-center ${card.status === 'FROZEN' ? 'bg-gray-100 text-gray-400' : 'bg-rose-50 text-rose-600'}`}>
                  {showDetails ? <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5}><path strokeLinecap="round" strokeLinejoin="round" d="M13.875 18.825A10.05 10.05 0 0112 19c-4.478 0-8.268-2.943-9.543-7a9.97 9.97 0 011.563-3.029m5.858.908a3 3 0 114.243 4.243M9.878 9.878l4.242 4.242M9.88 9.88l-3.29-3.29m7.532 7.532l3.29 3.29M3 3l3.59 3.59m0 0A9.953 9.953 0 0112 5c4.478 0 8.268 2.943 9.543 7a10.025 10.025 0 01-4.132 5.411m0 0L21 21" /></svg> : <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5}><path strokeLinecap="round" strokeLinejoin="round" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z" /></svg>}
                </div>
                <span className={`text-xs font-black ${card.status === 'FROZEN' ? 'text-gray-400' : 'text-purple-950'}`}>
            ...
            <button onClick={startSetPin} className="w-full bg-white rounded-2xl py-4 flex items-center justify-center gap-3 border border-purple-50 shadow-sm hover:bg-purple-50 transition-colors active:scale-95">
              <div className="w-8 h-8 rounded-full bg-purple-50 flex items-center justify-center text-purple-600">🔒</div>
              <span className="text-purple-950 text-xs font-black uppercase tracking-wider">Set Card PIN</span>
            </button>
          </motion.div>
        )}
      </div>

      <Modal open={pinModalOpen} onClose={() => setPinModalOpen(false)} title="Card PIN">
        <div className="py-2 px-2 relative z-[99999]">
          <AnimatePresence mode="wait">
            {pinStep === 'NEW' && (
              <motion.div key="new" initial={{opacity:0, x:20}} animate={{opacity:1, x:0}} exit={{opacity:0, x:-20}} className="text-center pb-4">
                 <p className="text-purple-950 text-sm font-bold mb-4">Set a 4-digit PIN for your Virtual Card</p>
                 <PinInput label="" onComplete={handleNewPinComplete} onReset={pinReset} />
              </motion.div>
            )}

            {pinStep === 'CONFIRM' && (
              <motion.div key="confirm" initial={{opacity:0, x:20}} animate={{opacity:1, x:0}} exit={{opacity:0, x:-20}} className="text-center pb-4">
                 <p className="text-purple-950 text-sm font-bold mb-4">Confirm your Virtual Card PIN</p>
                 <PinInput label="" onComplete={handleConfirmPinComplete} onReset={pinReset} />
              </motion.div>
            )}
          </AnimatePresence>
        </div>
